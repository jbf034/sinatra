...
Sinatra 是一个使用ruby 快速创建web 应用程序 的 DSL
...
它也建议运行  gem install thin,

路由

在Sinatra 一个路由是http方法配对一个url路径模式，每个路由都和块相关联
...
路由模式也可以包括通配符，通过params[:splat]数组参数调用
...
随便说下，如果你的路径遍历保护失效，那访问路径在你配置之前可能被修改
...
also accepted.
（通过路由中的块做为返回给客户的主要部分，或是通过中间架构，通常反回为字符串，如上面例子，
但也有其它方式）

You can return any object that would either be a valid Rack response, Rack
body object or HTTP status code:
（你可以返回任何对象，或是一个框架的回应，框架对象或是http状态码）
